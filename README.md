...
|--------|----------------|--------------------|-----------|--------------------|
| A. Full LoRA (Baseline) | FP16 training | None | Qwen 1.5B | FP16 |
| B. LoRA + PTQ | FP16 training | Post-training (int8/int4) | Qwen 1.5B | FP16 |
| C. QLoRA | 4-bit NF4 training (quantization-aware) | During training | Qwen 1.5B | FP16 |
...
| **Compute Type** | Local GPU nodes; no TPU or cloud cluster dependency |
| **Software** | PyTorch, Hugging Face Transformers, PEFT, bitsandbytes, AutoGPTQ, FlashAttention-2 |
| **Existing Codebase** | LongRefiner GitHub repository for teacher model and preprocessing scripts |
| **Dataset** | - DQA: Natural Questions (NQ), TriviaQA queries  <br> - ADR: Same datasets with teacher-generated document outlines (50k samples)  <br> - Teacher supervision: 3B LongRefiner outputs |
| **Quantization Configs** | - PTQ: int8 / int4 quantization via AutoGPTQ (calibrated subset)  <br> - QLoRA: NF4 4-bit quantization, compute_dtype = bfloat16 |
...
|------|----------|--------------|
| Single-hop | NQ, TriviaQA | Fact-based QA tasks |
| Multi-hop | HotpotQA, 2Wiki | Require multi-step reasoning |
| Long-form | ASQA, ELI5, PopQA | Require extended contextual retrieval and synthesis |

All experiments are performed under a **2k token context limit**, following the LongRefiner paper setup, while the full-content baseline (unlimited tokens) is used for reference only.
...
| **Power Draw (W)** | Average GPU power consumption during training |
...
| **Energy per Query (J)** | Total energy consumed per query (Power × Time) |
...
1. **Performance Retention:**  
   F1 difference between Full LoRA and QLoRA/LoRA+PTQ.

2. **Efficiency Gains:**  
   Relative reduction in training VRAM, inference latency, and total power usage.

3. **Pareto Frontier Visualization:**  
   Plot *Accuracy (F1)* vs. *Efficiency (VRAM/Latency)* to highlight optimal trade-offs.
...
### **f. Expected Outcomes**
- **QLoRA** is expected to retain ≥ 98 % of full-precision accuracy while reducing both training and inference memory by **60–70 %**.  
...
All models will run on a single consumer GPU (e.g., RTX 3060) to demonstrate deployability on edge hardware.
...
- A detailed comparison of Full LoRA, LoRA+PTQ, and QLoRA on efficiency and accuracy.  
- Pareto frontier plots of performance–efficiency trade-offs.  
- A lightweight, deployable LongRefiner student model.  
- Final report analyzing when quantization-aware training is most beneficial for structured reasoning tasks.
